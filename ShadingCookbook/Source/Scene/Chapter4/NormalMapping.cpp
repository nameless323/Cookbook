...
NormalMapping::NormalMapping() : _angle(0), _prevTime(0), _autorotate(1), _rotateLeft(0), _rotateRight(0)
{
}

void NormalMapping::ProcessInput(int key, int action)
{
...
void NormalMapping::InitScene()
{
	...
	_angle = glm::radians(100.0f);

	_shader.SetUniform("Light.Intensity", vec3(0.9f, 0.9f, 0.9f));

	GLint w, h;
	glActiveTexture(GL_TEXTURE0);
	GLubyte* data = TGA::Read("./media/texture/ogre_diffuse.tga", w, h);
	...
	data = TGA::Read("./media/texture/ogre_normalmap.tga", w, h);
...
void NormalMapping::Shutdown()
{
	delete _ogre;
}

void NormalMapping::SetMatrices()
{
...
void NormalMapping::Resize(int w, int h)
{
	glViewport(0, 0, w, h);
	_width = w;
	_height = h;
...
void NormalMapping::CompileAndLinkShader()
{
	try
	{
