#include "DirectionalPhong.h"
#include <gtx/transform.hpp>
#include <iostream>
#include <GLFW/glfw3.h>
using glm::vec3;

DirectionalPhong::DirectionalPhong() : _angle(0), _prevTime(0), _autorotate(1), _rotateLeft(0), _rotateRight(0), _isDirectional(0)
{
}

void DirectionalPhong::ProcessInput(int key, int action)
{
	if (key == GLFW_KEY_ENTER && action == GLFW_PRESS)
		_autorotate = !_autorotate;
	if (key == GLFW_KEY_L && action == GLFW_PRESS)
		_isDirectional = !_isDirectional;
...
void DirectionalPhong::InitScene()
{
	...
	glm::vec4 worldLight = vec4(2.0f, 4.0f, 2.0f, _isDirectional);
...
void DirectionalPhong::Render()
{
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	//glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	glm::vec4 worldLight = glm::rotate(_angle / 30.0f, vec3(1.0f, 0.0f, 0.0f)) * vec4(2.0f, 4.0f, 2.0f, _isDirectional);
...
void DirectionalPhong::Update(float t)
{
...
void DirectionalPhong::Shutdown()
{
	delete _teapot;
}

void DirectionalPhong::SetMatrices()
{
...
void DirectionalPhong::Resize(int w, int h)
{
	glViewport(0, 0, w, h);
	_width = w;
	_height = h;
...
void DirectionalPhong::CompileAndLinkShader()
{
	try
	{
		_shader.CompileShader("Shaders/DirectionalPhong/DirectionalPhong.vert");
		_shader.CompileShader("Shaders/DirectionalPhong/DirectionalPhong.frag");
		_shader.Link();
		_shader.Validate();
		_shader.Use();
	}
