#include "Discarding.h"
#include <gtx/transform.hpp>
#include <iostream>
#include <GLFW/glfw3.h>
using glm::vec3;

Discarding::Discarding() : _angle(0), _prevTime(0), _autorotate(1), _rotateLeft(0), _rotateRight(0)
{
}

void Discarding::ProcessInput(int key, int action)
{
...
void Discarding::Update(float t)
{
...
void Discarding::Shutdown()
{
	delete _teapot;
}

void Discarding::SetMatrices()
{
...
void Discarding::Resize(int w, int h)
{
	glViewport(0, 0, w, h);
	_width = w;
	_height = h;
...
void Discarding::CompileAndLinkShader()
{
	try
	{
		_shader.CompileShader("Shaders/Discarding/Discarding.vert");
		_shader.CompileShader("Shaders/Discarding/Discarding.frag");
		_shader.Link();
		_shader.Validate();
		_shader.Use();
	}
